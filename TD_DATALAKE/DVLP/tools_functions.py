...
# ==============================================================================
def log_message_maker(data_list):
    path_log = "C:/TD_DATALAKE/LOGFILES/"
    file_log = path_log + "logfiles.txt"
...
def deplacement_fichier(path_source,path_destinatation, zone="landing_zone"):  
    ...
    t = datetime.now()
    dhm = str(t.date()) + " à " + str(t.hour) + "h" + str(t.minute)
    if zone == "landing_zone":
        titre = "----------- DEPLACEMENT FICHIERS VERS LANDING ZONE "
    else:
        titre = "----------- DEPLACEMENT FICHIERS VERS CURATED ZONE "

    log_data = titre + dhm + "-----------" + "\n" +\
               " - Nombre de fichiers LINKEDIN EMP déplacé : " + str(nb_link_emp) + "\n" +\
               " - Nombre de fichiers GLASSDOR AVI déplacé : " + str(nb_glassdor_avis) + "\n" +\
               " - Nombre de fichiers GLASSDOR SOC déplacé : " + str(nb_glassdor_soc) + "\n" +\
               "----------- FIN-----------" + "\n"

    log_message_maker(log_data)
...
def creation_metadata_technique(path_source_data,path_landing_zone):
    ...
    t = datetime.now()
    dhm = str(t.date()) + " à " + str(t.hour) + "h" + str(t.minute)
    log_data = "Créatioon métadonnées techniques le : " + dhm + "\n"
    log_message_maker(log_data)
...
def creation_metadata_descriptif(path_landing_zone,path_curated_zone):     
    ...
    t = datetime.now()
    dhm = str(t.date()) + " à " + str(t.hour) + "h" + str(t.minute)
    log_data = "Créatioon métadonnées descriptives le : " + dhm + "\n"
    log_message_maker(log_data)